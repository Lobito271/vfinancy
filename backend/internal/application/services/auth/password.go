...
func ValidatePasswordStrength(password string) error {
	if len(password) < 8 {
		return fmt.Errorf("VALIDATION: la contraseña debe tener al menos 8 caracteres")
	}
	...
	if !hasUpper {
		return fmt.Errorf("VALIDATION: la contraseña debe contener al menos una letra mayúscula")
	}
	if !hasLower {
		return fmt.Errorf("VALIDATION: la contraseña debe contener al menos una letra minúscula")
	}
	if !hasDigit {
		return fmt.Errorf("VALIDATION: la contraseña debe contener al menos un dígito")
	}
