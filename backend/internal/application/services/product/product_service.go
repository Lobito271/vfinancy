...
// ProductService owns the product workflow.
...
func New(repo repositories.ProductRepository, txm services.TxManager, log *common.Logger) *ProductService {
...
// domain constructor.
func (s *ProductService) Create(ctx context.Context, in CreateInput) (*masterdata.Product, error) {
	var out *masterdata.Product
	err := s.txm.WithinTransaction(ctx, func(ctx context.Context) error {
		uow := repositories.UnitOfWorkFromContext(ctx)
		p, err := masterdata.NewProduct(time.Now().UTC(), masterdata.NewProductOptions{
		...
		if err := uow.Products().Create(ctx, p); err != nil {
			return err
		}
		out = p
		return nil
...
func (s *ProductService) UpdateCost(ctx context.Context, id uuid.UUID, cost valueobjects.Money) error {
	err := s.txm.WithinTransaction(ctx, func(ctx context.Context) error {
		uow := repositories.UnitOfWorkFromContext(ctx)
		p, err := uow.Products().GetByID(ctx, id)
		if err != nil {
			return err
		}
		...
		return uow.Products().Update(ctx, p)
	})
	if err != nil {
		return err
	}
