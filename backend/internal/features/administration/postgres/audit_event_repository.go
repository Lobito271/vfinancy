...
func NewAuditEventRepository(db *sql.DB) *auditEventRepository {
	return &auditEventRepository{q: persistence.FromDB(db)}
}
...
func (r *auditEventRepository) Create(ctx context.Context, e *administration.AuditEvent) error {
	...
	_, err := persistence.Q(ctx, r.q).ExecContext(ctx, q,
		e.ID, e.CompanyID, persistence.NullIfEmptyUUID(e.UserID), persistence.NullIfEmptyUUID(e.SessionID),
		string(e.EventType), persistence.NullIfEmpty(e.TargetType), persistence.NullIfEmptyUUID(e.TargetID),
		e.Description, meta, e.IPAddress, e.Device, e.OccurredAt,
	)
	return persistence.Translate(err)
}

func (r *auditEventRepository) List(ctx context.Context, companyID uuid.UUID, filter administration.AuditEventFilter) ([]*administration.AuditEvent, int, error) {
	var (
	...
	copy(countArgs, args)
	if err := persistence.Q(ctx, r.q).QueryRowContext(ctx, "SELECT count(*) FROM audit_events WHERE "+where, countArgs...).Scan(&total); err != nil {
		return nil, 0, persistence.Translate(err)
	}
	...
	rows, err := persistence.Q(ctx, r.q).QueryContext(ctx, q, args...)
	if err != nil {
		return nil, 0, persistence.Translate(err)
	}
...
func scanAuditEvent(row *sql.Row) (*administration.AuditEvent, error) {
	...
	if userID.Valid {
		id := persistence.ParseUUID(userID.String)
		e.UserID = &id
	}
	if sessionID.Valid {
		id := persistence.ParseUUID(sessionID.String)
		e.SessionID = &id
	}
	if targetID.Valid {
		id := persistence.ParseUUID(targetID.String)
		e.TargetID = &id
	}
	return e, nil
}
...
var _ administration.AuditEventRepository = (*auditEventRepository)(nil)
