...
func newCurrencyRepository(db *sql.DB) *currencyRepository {
	return &currencyRepository{q: &dbBox{db: db}}
}

func newCurrencyRepositoryTx(tx *sql.Tx) *currencyRepository {
	return &currencyRepository{q: &txBox{tx: tx}}
}
...
func (r *currencyRepository) Upsert(ctx context.Context, c *masterdata.Currency) error {
...
func scanCurrency(row *sql.Row) (*masterdata.Currency, error) {
	c := &masterdata.Currency{}
	var code, currencyType string
...
var _ repositories.CurrencyRepository = (*currencyRepository)(nil)
