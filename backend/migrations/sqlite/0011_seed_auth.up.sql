-- 0011_seed_auth.up.sql (SQLite)
-- Module 1: Authentication
-- Seed data for a fresh install. Idempotent: every INSERT is guarded so
-- re-running the migration is safe and an environment that already has
-- data is not broken. SQLite has no ON CONFLICT (unique_index) WHERE
-- clause, so:
--   - PK/unique-index conflicts use INSERT OR IGNORE;
--   - partial unique indexes (WHERE deleted_at IS NULL) use a
--     WHERE NOT EXISTS guard instead.
--
-- What gets seeded:
...
-- any real login is possible. A password reset is mandatory on first use.
