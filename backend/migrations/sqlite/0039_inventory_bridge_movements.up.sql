-- 0039_inventory_bridge_movements.up.sql (SQLite)
-- Module: Inventory
...
--
-- SQLite cannot ALTER a CHECK constraint, so the movements table is
-- rebuilt. Foreign keys must be off during the rebuild; the migration
-- runner wraps each file in a transaction (where PRAGMA foreign_keys
-- is a no-op), so we end that transaction first and reopen it at the
-- end.
