-- 0044_force_seed_cards_and_warehouse.up.sql (SQLite)
...
SELECT lower(hex(randomblob(16))), c.id, '165.01',
...
SELECT lower(hex(randomblob(16))), c.id, s.issuer, s.last_four,
...
SELECT lower(hex(randomblob(16))), c.id, 'SUC-01', 'Sucursal Principal', TRUE
  FROM companies c
 WHERE c.code <> 'SETUP'
   AND NOT EXISTS (
